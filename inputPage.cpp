...
{
  QFileDialog fd (this, tr ("cpfFilter Input files"));
  ...
  fd.setNameFilter (QString ("CZMIL (*.cpf)"));
...
{
  QString title = tr ("cpfFilter Input directories");
              ...
              if (files_dir.cd (dirName))
                {
                  ...
                  for (int32_t k = 0 ; k < sub_flist.size () ; k++)
                    {
                      inputFiles->append (sub_flist.at (k).absoluteFilePath ());
                    }
                }
            }
        }
